...
/home/user/.local/python-3.9.13/bin/python3.9 -m pip install httpx prettytable playwright retrying

/home/user/.local/python-3.9.13/bin/python3.9 -m playwright install firefox
```

3.运行

```
/home/user/.local/python-3.9.13/bin/python3.9 quark.py
```
...
[quark_login.py#L34](./quark_login.py#L34)

```shell
with sync_playwright() as p:

    # for google chrome
    self.context = p.chromium.launch(channel="chrome", headless=False)
...
[quark.py#L771](quark.py#L771)
```py
║     1.转存文件   2.生成分享链接   3.切换网盘保存目录   4.创建网盘文件夹   5.下载到本地   6.登录      ║
```
